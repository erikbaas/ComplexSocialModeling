...
to go
  ...
  ;; NEW: reset our per-tick variables
  set last-period-output this-period-output
  set this-period-output 0
  set this-period-deaths 0
  ...
  ask turtles [
    ...
    if sugar <= 0 or age > max-age [
      ...
      set this-period-deaths (this-period-deaths + 1) ;; NEW
      die
    ]
    run visualization
  ]
...
to turtle-eat ;; turtle procedure
  ...
  ;; NEW: sugar magically grows in proportion to the amount we have
  set sugar (sugar + (sugar * return-on-capital / 100))
  
  ;; metabolize some sugar, and eat all the sugar on the current patch
  set sugar (sugar - metabolism + psugar)
  
  ;; NEW: increment last-output by amount of sugar harvested (NOT net gain in sugar)
  set this-period-output (this-period-output + psugar)
...
to update-lorenz-and-gini
  ...
  ;; NEW: let changed to set (global variable)
  ...
  ;; NEW: update growth rate
  ifelse last-period-output = 0
    [ set growth-rate 0 ]
    [ set growth-rate (this-period-output - last-period-output) / last-period-output * 100 ]

  ;; NEW: update death rate
  set death-rate this-period-deaths / num-people * 100
    
  ;; NEW: redistribute wealth
  let per-capita-transfer ((wealth-tax-rate / 100) * (total-wealth / num-people))
  ask turtles [
    let transfer (per-capita-transfer - sugar * wealth-tax-rate / 100)
    set sugar (sugar + transfer)
  ]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "ifelse growth-rate > 10 [ plot 10 ] [ ifelse growth-rate < -10 [ plot -10 ] [ plot growth-rate ] ]"
